...
from dialog import Dialog, ListDialog, name_popup, reset_password
...
class CafeNallaApp(MDApp):
	...
	def on_start(self):
		...
		# Bind device buttons to functions
		Window.bind(on_keyboard=self.on_key)
		#runTouchApp(MyKeyboardListener())

	def on_key(self, window, key, *args):
		if key == 27:    # This represents the back key or the esc key
			# First check if keyboard is active or open
	...
	def login(self, **kwargs):
		...
		def check_login(f_name, p_word, current_screen, value):
			...
			spinner_grid = self.root.ids["sign_up_screen"].ids["main_floatie"]
			...
			thread = threading.Thread(target=check_logic2,args=(f_name, p_word, current_screen, spinner_tools))

			toast('logging in...')
			thread.start()
		
		def check_logic2(f_name, p_word, current_screen, spinner_tools):
		...
		def OTPPop(val):
			...
			def confirm_passwords(pop, passwords, user_cached, user_id, email, spinner, val):
			...
			def confirm_pin(pop, input_field, random_val, time_up, found_local, user_id, email, spinner,val):
				plausable_pin = False
				try:
				...
				if plausable_pin == True:
					if time_up == True:
						...
						if pin == random_val:
							...
							confirm_btn.bind(on_press=partial(confirm_passwords, reset_pass, passwords, 
							                                  found_local, user_id, email, spinner))
							cancel_btn.bind(on_press=partial(cancel, reset_pass))
							'''
							WEAK REF ERROR
			...
			def confirm_email(pop, input_field, val):
				def start_e_confirmation(pop, input_field, spinner):
								...
								if isinstance(found_email, int): 
									...
									if connected == True:
										...
										if sent_email == True:
											...
											confirm_pin_btn.bind(on_press=partial(confirm_pin, verify_pin, pin, random_val, time_up, found_local, found_email, email_text, spinner))
											cancel_pin_btn.bind(on_press=partial(cancel, verify_pin))
											verify_pin.open()
				...
				t = threading.Thread(target=start_e_confirmation, args=(pop, input_field, spinner_tools))
				t.start()
			...
			confirm_email_btn.bind(on_press=partial(confirm_email, verify_email, email))
			cancel_email_btn.bind(on_press=partial(cancel, verify_email)) 
			verify_email.open()
			'''
			WEAK REF ERROR
	...
	def sign_up(self):
		...
		def check_sign_up(f_name, l_name, e_mail, p_word, value):
			...
			thread = threading.Thread(target=check_sign_up2,args=(f_name, l_name, e_mail, p_word, spinner_tools))

			toast('creating account...')
			thread.start()

		def check_sign_up2(f_name, l_name, e_mail, p_word, spinner_tools):
			# A python requirement for calling functions inn different classes
			cred = Credentials()
